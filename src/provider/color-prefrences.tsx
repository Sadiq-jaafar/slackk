...
const ColorPrefrences = createContext<ColorPrefrencesContext | undefined>(
  undefined
);

export const useColorPrefrences = () => {
  const context = useContext(ColorPrefrences);
  if (!context) {
    throw new Error(
...
export const ColorPrefrencesProvider: FC<{ children: ReactNode }> = ({
  ...
  const value: ColorPrefrencesContext = { color, selectColor };

  if (!isMounted) return null;

  return (
    <ColorPrefrences.Provider value={value}>
      {children}
    </ColorPrefrences.Provider>
  );
};
